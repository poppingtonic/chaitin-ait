...
/* Credit: Gregory Chaitin - The Limits of Mathematics (Tutorial Version) 1995 */
/*
    The storage required by this interpreter is 8*4 = 32 bytes times
    the symbolic constant SIZE, which is 32 * 1000000 = 32 megabytes.
    To run this interpreter in small machines, reduce the 
    #define SIZE 1000000 below.
...
long car[SIZE];
long cdr[SIZE]; /* tree storage */
short atom[SIZE]; /* is it an atom? */
short numb[SIZE]; /* is it a number? */
/* The following is only used for atoms */
long vlst[SIZE]; /* bindings for each atom */
long pname[SIZE]; /* print name for each atom = list of characters in reverse */
...
long obj_list; /* list of all atoms (& every other tokens read except numbers) */
...
long col = 0; /* column in each 50-char chunk of output (preceded by 12-char prefix) */
time_t time1; /* clock at time of execution */
time_t time2; /* clock at the end of execution */
...
long mk_atom(long number, char *name, long args)
{
    long a;
    a = cons(nil, nil); /* get an empty node */
    car[a] = cdr[a] = a; /* so that car and cdr of atom = atom */
    atom[a] = 1;
    numb[a] = 0;
...
{ /* digits are in reverse order, and 0 has empty list of digits */
    ...
    if necessary before calling this routing ( to avoid removing leading zeros unnecessarily)
    */
    pf_numb[a] = 0;
    pf_args[a] = 0;
    vlst[a] = 0;
...
long mk_string(char *p) /* make list of chars in reverse order */
{
    long v = nil;
    while (*p != '\0')
        v = cons(*p++, v);
...
long cons(long x, long y) /* get free node and stuff x & y in it */
{
    long z;
...
{
    printf("%-12s", x);
    col = 0; /* so can insert \n and 12 blanks every 50 chars of output */
    out_lst(y);
    printf("\n");
    return y;
}
...
void out_lst(long x) /* output list */
{
    if(numb[x] && pname[x] == nil) { out_chr('0'); return;} /* null list of digits = zero */
...
{
    if(x == nil) return;
    out_atm(cdr[x]); /* output chars in reverse order */
    out_chr(car[x]);
}
...
long eq_wrd(long x, long y) /* are two lists of chars equal */
{
...
long lookup_word(long x) /* is word in the object list? */
{
    long i = obj_list;
    while(!atom[i]) {
        ...
        while(line != nil){
            ...
            if(character == ' ' || character == '\n' ||
               ...
               character == '\'' || character == '\"'){
                ...
                word = nil;
                /* add break character to word (in reverse order) */
                /* keep only nonblank printable ASCII codes */
                if(32 < character && character < 127)
                word = cons(character, word);
               }
        }
        buffer = cdr[buffer]; /* remove stub at the beginning of buffer */
...
long eval(long e, long d) /* evaluate expression */
{
    ...
    */
    ...
    switch(pf_numb[f]) {
        ...
        case 14: return mk_numb(add(nmb(x), nmb(y), 0)); /* no carry in initially */
        case 15: return mk_numb(mult(nmb(x), nmb(y)));
        case 16: return mk_numb(expon(nmb(x), nmb(y)));
        case 17: if(compare(nmb(x), nmb(y)) != '>') return mk_numb(nil); /* y too big to subtract from x */
                 else return mk_numb(remove_leading_zeros(sub(nmb(x), nmb(y), 0))); /* no borrow in initially */
        case 18: return mk_numb(base2to10(x)); /* bit-string to decimal */
        case 19: return base10to2(nmb(x)); /* decimal to bit-string */
    ...
    if(f ==  wrd_try) {
        long stub, old_try_has_smaller_time_limit = 0; /* normal case that x < d */
        ...
        if(x == wrd_no_time_limit || (d != wrd_no_time_limit && compare(x, d) != '<')){
            ...
            x = d; /* continue to uise older, more constraining time limit */
        }
...
void clean_env(void) /* clean environment */
{
    ...
    while(o != nil) {
        var = car[o];
        vlst[var] = cons(var, vlst[var]); /* everything evals to self */
        o = cdr[o];
    }
    car[vlst[wrd_nil]] = nil; /* except the value of nil is () */
}
...
long length(long x) /* # of elements in it */
{
...
long add(long x, long y, long carry_in)
{
    ...
    if(sum <= '9') return cons(sum, add(rest1, rest2, 0));
    return cons(sum-10, add(rest1, rest2, 1));
}
...
long sub(long x, long y, long borrow_in) /* x - y assumes x >= y */
{
    ...
    if(difference >= '0') return cons(difference, sub(rest1, rest2, 0));
    return cons(difference+10, sub(rest1, rest2, 1));
}

long mult(long x, long y) /* goes faster if x is small */
{
    long sum = nil;
    if(y == nil) return nil; /* otherwise produces result 0000 */
    while(x != nil) {
        long digit = car[x];
        while(digit-- > '0') sum = add(sum, y, 0);
        x = cdr[x];
...
long expon(long base, long exponent)
{
    long product = cons('1', nil);
    while(expon != nil) {
        product = mult(base, product); /* multiply faster if smaller comes first */
...
long base2to10(long x) /* convert bit string to decimal number */
{
    ...
    while(!atom[x]) {
        ...
        result = add(result, result, next_bit);
    }
    return result;
}
...
void write_chr(long x) /* convert char to list of 8 bits */
{
...
{
    if(numb[x] && pname[x] == nil) { write_chr('0'); return;} /* null list of digits = zero */
...
{
    if(x == nil) return;
    write_atm(cdr[x]); /* output chars in reverse order */
    write_chr(car[x]);
}
        ...
        if(character == ' ' || character == '\n' ||
           ...
           { /* add nonempty word to end of buffer */
            ...
            end_of_buffer = cdr[end_of_buffer] = cons(cons(character, nil), nil);
               ...
               /* keep only nonblank printable ASCII characters */
               if(32 < character && character < 127)
               word = cons(character, word);
           }
    } /* end while(line != nil) */
...
long read_word(void){ /* Read word from Turing machine tape */
    ...
    /* first check if word consists only of digits */
    if(only_digits(word)) word = mk_numb(remove_leading_zeros(word));
    /* also makes 00099 into 99 and 000 into null */
    else word = lookup_word(word); /* look up word in object list */
    /* also does mk_atom and adds it to the object list if necessary */
    return word;
}
